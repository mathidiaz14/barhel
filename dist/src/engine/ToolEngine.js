...
export class ToolEngine {
    ...
    workdirReal;
    constructor(workdir = process.cwd(), autonomous = false, policies) {
        ...
        try {
            switch (action.type) {
                ...
                default:
                    return {
                        success: false,
                        error: `Acción desconocida: ${action.type}`,
                        output: '',
                    };
            }
        }
     ...
     */
    async writeFileWithConfirmation(relPath, content) {
        const fullPath = this.resolveSafePath(relPath);
        if (this.planOnly) {
     ...
     */
    createDiffPreview(relPath, newContent) {
        try {
     ...
     */
    async runCommandWithConfirmation(command) {
        if (this.planOnly) {
            return {
                success: true,
    ...
    scanDirRecursive(dir, depth, maxDepth) {
        if (depth > maxDepth)
            return [];
        const results = [];
        try {
    ...
    async glob(pattern, relPath) {
        ...
        try {
            ...
            if (safeMatches.length > MAX_ENTRIES)
     ...
     */
    async requestUserConfirmation(question) {
     ...
     */
    assertInsideWorkdir(resolved, original) {
        const relCheck = (base, target) => {
     ...
     */
    async executeWebhook(url, method = 'POST', headers, body) {
        if (!url) {
