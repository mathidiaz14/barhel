...
export interface CodeGraphData {
  ...
  callGraph: Record<string, string[]>; // symbol -> called symbols
  callerGraph: Record<string, string[]>; // symbol -> symbols that call it
}

export class CodeGraphEngine {
  private workdir: string;
  private cachePath: string;
  private graph: CodeGraphData;
   ...
   */
  public async ensureLoaded(forceRescan = false): Promise<CodeGraphData> {
   ...
   */
  private parseFile(relPath: string, content: string, hash: string): FileNode {
    ...
    if (['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs'].includes(ext)) {
      for (let i = 0; i < lines.length; i++) {
        ...
        if (classMatch) {
          ...
          if (line.includes('export ')) exports.push(name);
        }
        ...
        if (fnName && fnName.length > 1) {
          ...
          if (line.includes('export ')) exports.push(fnName);
        }
      }
    } else if (ext === '.py') {
   ...
   */
  private extractCalls(block: string): string[] {
  ...
  private collectSourceFiles(dir: string): string[] {
    ...
    try {
      ...
      for (const entry of entries) {
        if (CodeGraphEngine.IGNORED_DIRS.has(entry.name)) continue;
   ...
   */
  public getCallers(symbolName: string): string[] {
   ...
   */
  public getCallees(symbolName: string): string[] {
  ...
  public getHierarchy(): string {
    ...
    for (const [file, node] of Object.entries(this.graph.files)) {
      if (node.symbols.length === 0) continue;
      lines.push(`📄 ${file}`);
   ...
   */
  public inspectSymbol(symbolName: string): string {
    ...
    for (const s of syms) {
      ...
      if (s.params) lines.push(`  Parámetros: (${s.params.join(', ')})`);
    }
