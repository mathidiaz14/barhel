...
export class MistralDriver extends BaseDriver {
    ...
    async sendMessage(prompt, onChunk) {
        if (!this.page)
            throw new Error('Driver Mistral no está inicializado.');
        await this.ensureChatPage();
        const inputSelector = await this.findFirstVisibleSelector(this.config.selectors.inputPrompt, 10000);
        if (!inputSelector) {
            throw new Error('No se encontró el campo de entrada de Mistral. Inicia sesión con: barhel login mistral');
        }
...
//# sourceMappingURL=MistralDriver.js.map