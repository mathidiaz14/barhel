...
import { HistoryManager, ChatSession } from '../utils/history.js';
...
export class SessionManager {
  ...
  public getSession(sessionId: string): Orchestrator | undefined {
  ...
  public changeWorkdir(sessionId: string, newWorkdir: string): { ok: boolean; workdir?: string; error?: string } {
    const orch = this.getSession(sessionId);
    if (!orch) return { ok: false, error: 'Sesión no encontrada' };
    try {
  ...
  private toManaged(orch: Orchestrator): ManagedSession {
   ...
   */
  public async runTurn(sessionId: string, prompt: string): Promise<CommandResult> {
    const orch = this.orchestrators.get(sessionId);
    if (!orch) return { ok: false, output: '', message: 'Sesión no encontrada.' };
    if (!prompt.trim()) return { ok: false, output: '', message: 'Prompt vacío.' };
  ...
  public async interrupt(sessionId: string): Promise<CommandResult> {
    const orch = this.orchestrators.get(sessionId);
    if (!orch) return { ok: false, output: '', message: 'Sesión no encontrada.' };
    if (!orch.isTurnRunning) return { ok: true, output: 'No hay ningún turno en ejecución.' };
  ...
  public async closeSession(sessionId: string): Promise<CommandResult> {
    const orch = this.orchestrators.get(sessionId);
    if (!orch) return { ok: false, output: '', message: 'Sesión no encontrada.' };
    await orch.shutdown();
  ...
  public async command(sessionId: string, cmd: string, arg: string): Promise<CommandResult> {
    const orch = this.getSession(sessionId);
    if (!orch) return { ok: false, output: '', message: 'Sesión no encontrada.' };
    ...
    switch (cleanCmd) {
      ...
      case 'explain':
        if (!arg.trim()) return { ok: false, output: '', message: 'Uso: /explain <simbolo o archivo>' };
      ...
      case 'title':
        if (!arg.trim()) return { ok: false, output: '', message: 'Uso: /title <nuevo titulo>' };
      ...
      case 'status': {
        ...
        const lines = Object.entries(status).map(([p, info]) =>
          `• ${p.toUpperCase().padEnd(12)}: ${info.exists ? '✔ CONECTADO (' + info.fileCount + ' archivos)' : '✖ SIN SESIÓN'}`
        );
      ...
      case 'workdir':
        if (!arg) return { ok: false, output: '', message: 'Uso: /workdir <ruta absoluta o relativa>' };
        try {
          orch.setWorkdir(arg);
  ...
  private async runDoctor(orch: Orchestrator, arg: string): Promise<CommandResult> {
  ...
  private async listSkills(): Promise<CommandResult> {
  ...
  private async handleSkill(arg: string): Promise<CommandResult> {
    const { SkillManager } = await import('../skills/SkillManager.js');
  ...
  private async getProgressReport(sessionId: string): Promise<CommandResult> {
    try {
  ...
  private async getWorkersAnalysis(): Promise<CommandResult> {
    ...
    const lines = records.map((r, i) =>
      `#${i + 1} [${r.workerName.toUpperCase()}] (${r.durationMs || 0}ms)\nTarea: ${r.subtaskPrompt}\nResultado:\n${r.fullResponse}\n───────────────────────────`
    );
    return {
      ok: true,
  ...
  private async handleTelegram(orch: Orchestrator, arg: string): Promise<CommandResult> {
  ...
  private async handleDaemon(orch: Orchestrator, arg: string): Promise<CommandResult> {
  ...
  private async handleBackup(arg: string): Promise<CommandResult> {
  ...
  private async handleRestore(arg: string): Promise<CommandResult> {
    if (!arg.trim()) {
  ...
  private async handleImportSessions(): Promise<CommandResult> {
    ...
    const lines = results.map((r) => {
      if (r.skipped) return `• ⏭ ${r.provider}: ${r.message}`;
      if (r.success) return `• ✔ ${r.provider}: ${r.message}`;
  ...
  private async handleClearSessions(arg: string): Promise<CommandResult> {
  ...
  private handleMemory(workdir: string, arg: string): CommandResult {
    const parts = (arg || '').trim().split(/\s+/);
    const sub = parts[0]?.toLowerCase();
    
    if (sub === 'add') {
      const fact = parts.slice(1).join(' ').trim();
      if (!fact) return { ok: false, output: '', message: 'Uso: /memory add <hecho>' };
    ...
    if (sub === 'remove') {
      ...
      if (isNaN(idx)) return { ok: false, output: '', message: 'Uso: /memory remove <numero>' };
    ...
    if (sub === 'list' || !sub) {
      ...
      if (entries.length === 0) return { ok: true, output: 'La memoria está vacía.' };
  ...
  private handlePromptLibrary(orch: Orchestrator, arg: string): CommandResult {
    const parts = (arg || '').trim().split(/\s+/);
    const sub = parts[0]?.toLowerCase();
    ...
    if (sub === 'save') {
      ...
      if (!name || !text) return { ok: false, output: '', message: 'Uso: /prompt save <nombre> <texto>' };
    ...
    if (sub === 'remove') {
      const name = parts[1];
      if (!name) return { ok: false, output: '', message: 'Uso: /prompt remove <nombre>' };
    ...
    if (sub === 'list' || !sub) {
      ...
      if (entries.length === 0) return { ok: true, output: 'No hay prompts guardados.' };
    ...
    if (sub === 'run') {
      const name = parts[1];
      if (!name) return { ok: false, output: '', message: 'Uso: /prompt run <nombre>' };
      const entry = PromptLibrary.get(workdir, name);
      if (!entry) return { ok: false, output: '', message: `Prompt "${name}" no encontrado.` };
  ...
  private async handleBranch(workdir: string, arg: string): Promise<CommandResult> {
    const parts = (arg || '').trim().split(/\s+/);
    const sub = parts[0]?.toLowerCase();

    if (sub === 'new') {
      const name = parts[1];
      if (!name) return { ok: false, output: '', message: 'Uso: /branch new <nombre>' };
    ...
    if (sub === 'switch') {
      const name = parts[1];
      if (!name) return { ok: false, output: '', message: 'Uso: /branch switch <nombre>' };
  ...
  private async handleGithub(workdir: string, arg: string): Promise<CommandResult> {
    if (!arg) return { ok: false, output: '', message: 'Uso: /github <comando gh>' };
  ...
  private async handleMcp(workdir: string, arg: string): Promise<CommandResult> {
    if (!arg) return { ok: false, output: '', message: 'Uso: /mcp <start|stop> <server>' };
    const parts = arg.trim().split(/\s+/);
    if (parts[0] === 'start') {
  ...
  private handleContext(workdir: string, arg: string): CommandResult {
    const parts = arg.trim().split(/\s+/);
    const action = parts[0];
    const file = parts[1];

    if (!action) return { ok: false, output: '', message: 'Uso: /context <add|remove|list> [archivo]' };

    if (action === 'add') {
      if (!file) return { ok: false, output: '', message: 'Especifica la ruta relativa del archivo a agregar.' };
      ...
      if (!file) return { ok: false, output: '', message: 'Especifica la ruta relativa del archivo a remover.' };
  ...
  private getHelpText(): CommandResult {
    return {
      ok: true,
...
🩺 DIAGNÓSTICO & AUTENTICACIÓN
  ...
  private async runTests(orch: Orchestrator, arg: string): Promise<CommandResult> {
  ...
  private async runGraph(orch: Orchestrator, arg: string): Promise<CommandResult> {
    ...
    if (query) {
      ...
      if (!info.includes('no encontrado')) return { ok: true, output: info };
  ...
  private async commit(orch: Orchestrator, arg: string): Promise<CommandResult> {
    try {
  ...
  private async newSession(orch: Orchestrator, arg: string): Promise<CommandResult> {
