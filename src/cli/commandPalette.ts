...
export class CommandPalette {
  ...
  private readonly onExecute: (command: string, args: string) => Promise<void>;
  private filteredCommands: CommandItem[] = [];
  private selectedIndex = 0;
  private inputBuffer = '';
  private isOpen = false;
  private isFinishing = false;
  private stdinListener: ((key: Buffer) => void) | null = null;
  ...
  public open(initialInput = ''): Promise<void> {
  ...
  private async executeSelected(): Promise<void> {
    ...
    const selectedMatchesTypedCommand = selected && (
      !command ||
      command === '/' ||
