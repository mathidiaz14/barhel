 ...
 */
function findBrowserProfile(browserName) {
    ...
    for (const candidate of candidates) {
        if (fs.existsSync(candidate)) {
            ...
            try {
                ...
                const hasProfile = files.some(f => ['Cookies', 'Preferences', 'Local Storage', 'Session Storage'].includes(f));
                if (hasProfile)
                    return candidate;
            }
            catch {
 ...
 */
function copyDirSync(src, dest, maxDepth = 3) {
    if (maxDepth <= 0)
        return;
 ...
 */
export function importSessionsFromBrowser(providersToImport, browserName = 'chrome', force = false) {
    const results = [];
 ...
 */
function hasActiveSession(sessionDir) {
    try {
        if (!fs.existsSync(sessionDir))
            return false;
        const files = fs.readdirSync(sessionDir);
        return files.length > 2 && files.some(f => ['Cookies', 'Local Storage', 'Session Storage', 'IndexedDB', 'Network'].includes(f));
    }
    catch {
        return false;
    }
