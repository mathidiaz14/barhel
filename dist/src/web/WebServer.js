...
export class WebServer {
    ...
    handleWs(ws, url) {
        ws.send(JSON.stringify({ type: 'hello', sessionId: 'system', payload: { version: getBarhelVersion(), ts: new Date().toISOString() }, ts: new Date().toISOString() }));
    }
    ...
    async handleApi(pathname, url, req, res) {
        ...
        try {
            ...
            if (pathname === '/api/skills/install' && method === 'POST') {
                const body = await this.readBody(req);
                if (!body.url)
                    return this.sendJson(res, 400, { ok: false, error: 'URL requerida' });
                const { SkillManager } = await import('../skills/SkillManager.js');
            ...
            if (sessionMatch) {
                ...
                if (!sub && method === 'GET') {
                    ...
                    if (!sess)
                        return this.sendJson(res, 404, { ok: false, error: 'Sesión no encontrada' });
                    return this.sendJson(res, 200, {
                        ok: true,
                        sessionId,
                        session: {
                ...
                if (sub === 'turn' && method === 'POST') {
                    ...
                    if (!body.prompt || !String(body.prompt).trim())
                        return this.sendJson(res, 400, { ok: false, error: 'prompt requerido' });
            ...
            if (pathname === '/api/login/confirm' && method === 'POST') {
                ...
                if (!entry)
                    return this.sendJson(res, 400, { ok: false, error: `No hay un login activo para "${provider}".` });
    ...
    async handleLogin(provider, res) {
