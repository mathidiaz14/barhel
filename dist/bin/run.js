...
program
    ...
    const workers = options.workers ? options.workers.split(',').map((s) => s.trim()) : undefined;
    ...
    try {
        ...
        TUI.renderBanner(options.workdir, options.autonomous, leaderName, workersNames, orchestrator.getSessionTitle(), orchestrator.getSessionId());
        if (options.plan) {
