...
export class ContextManager {
  private static getContextFile(workdir: string): string {
    const dir = path.join(workdir, '.barhel');
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    return path.join(dir, 'context.json');
  }

  public static addFile(workdir: string, relPath: string): boolean {
    try {
  ...
  public static removeFile(workdir: string, relPath: string): boolean {
    try {
      const files = this.list(workdir);
  ...
  public static list(workdir: string): string[] {
    try {
  ...
  public static getContextString(workdir: string): string {
    const files = this.list(workdir);
