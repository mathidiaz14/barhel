 ...
 */
export async function execAsync(command: string, options: { cwd?: string; timeoutMs?: number; maxBuffer?: number } = {}): Promise<ExecResult> {
  ...
  try {
    ...
    let combined = '';
    if (stdoutStr) combined += `[STDOUT]\n${stdoutStr}\n`;
    if (stderrStr) combined += `[STDERR]\n${stderrStr}\n`;
