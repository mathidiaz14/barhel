...
export class DeepSeekDriver extends BaseDriver {
    ...
    async sendMessage(prompt, onChunk) {
        if (!this.page)
            throw new Error('Driver DeepSeek no está inicializado.');
        ...
        if (!inputSelector) {
            throw new Error('No se encontró el campo de entrada de DeepSeek. Si no has iniciado sesión, ejecuta primero: barhel login deepseek');
        }
    ...
    async waitForCompletion(previousCount = 0, onChunk) {
        if (!this.page)
            return;
    ...
    async extractLatestResponse() {
        ...
        try {
            const text = await this.page.evaluate(() => {
                ...
                const allMessages = Array.from(document.querySelectorAll('div[class*="message"], div[class*="chat"], article'));
